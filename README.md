...
| `npm run kompu [-- --sondeo --solo-indice --rubros=<ids> --estados=<lista> --max-paginas=N --presupuesto=N]` | Radar de **hardware TI para Kompu.cl** (tóner e insumos de impresión, impresoras/multifuncionales/escáneres/plotters, notebooks, UPS, CCTV) sobre los **cinco estados**, publicando `docs/kompu.html` — página nueva y separada que **no toca `docs/index.html`** salvo un enlace en su footer. Es **solo lectura**: no cotiza (no hay catálogo de costos de Kompu). Para medir el mercado **no pagina, cuenta**: el `total_resultados` de la primera página da el tamaño de la familia, así que los 5 estados completos cuestan ~72 requests y solo `publicada` se pagina. La tasa de éxito sale de esos totales corregidos por `precisionEnNombre`, **no de la muestra** —que lee ~25 filas por estado tenga 2.208 compras o 55, y daba un número 2,5× inflado—. `--sondeo` mide cada variante `q` antes de usarla (1 request c/u → `output/kompu-sondeo.md`). Acumula en `historico/kompu.jsonl` + `historico/kompu-censo.json` (versionados); `--solo-indice` reproduce la página idéntica con **0 requests**. |
